        ...
        @if (! empty($links))
            <div class="-mx-2 flex mt-4">
                @foreach($links as $text => $link)
                    <a href="{{ $link }}"
                        class="px-2 py-1.5 rounded-md text-sm font-medium focus:outline-none focus:ring-1 mr-3 border border border-orange-400 text-grey-700 hover:bg-orange-100/50 focus:ring-offset-orange-50 focus:ring-orange-500">
                        {{ $text }}
                    </a>
                @endforeach
            </div>
